...
def vonk2d(rseed,dx,dz,ax,az,ix,iz,pop,med,nu,vel,frac):
    ...
    data = np.random.random(size=(int(nz),int(nx)))
    ...
    rdata=rdata[0:int(nz-ddz-1), 0:int(nx-ddx-1)]

    return rdata, x, z, data, expcorr, rseed,dx,dz,ax,az,ix,iz,pop,med,D

def plotvonk2d(rseed,dx,dz,ax,az,ix,iz,pop,med,D,x,z,rdata):
    xdata = x
    ydata = z
    zdata = rdata
    mytitle = "rseed=" + str(rseed) + ", dx=dz=" + str(dx) + ", ax=" + str(ax) + ", az=" + str(az) + ", med=" + str(med) + ", pop=" + str(pop) + ", D=" + str(D)
    fig = go.Figure(data=go.Heatmap(x=xdata, y=ydata, z=np.flipud(zdata),
                                    colorscale='YlOrBr',
...
def plotAndCalcvonk2d(rseed,dx,dz,ax,az,ix,iz,pop,med,nu,vel,frac):
    ...
    print("Successfully plotted figure titled ", mytitle)
